...
describe('Google Maps map API tests', () => {
  ...
  it('should create marker different marker icon for start, end and normal position', () => {
    ...
    expect(GoogleMapsApi.getMarkerIcon.calls.mostRecent().args[0]).toBe(config.colorStart);
    expect(GoogleMapsApi.getMarkerIcon.calls.mostRecent().args[1]).toBeTrue();
    // when
    api.setMarker(1, track);
    // then
    expect(GoogleMapsApi.getMarkerIcon).toHaveBeenCalledTimes(2);
    expect(GoogleMapsApi.getMarkerIcon.calls.mostRecent().args[0]).toBe(config.colorNormal);
    expect(GoogleMapsApi.getMarkerIcon.calls.mostRecent().args[1]).toBeFalse();
    // when
    api.setMarker(2, track);
    // then
    expect(GoogleMapsApi.getMarkerIcon).toHaveBeenCalledTimes(3);
    expect(GoogleMapsApi.getMarkerIcon.calls.mostRecent().args[0]).toBe(config.colorStop);
    expect(GoogleMapsApi.getMarkerIcon.calls.mostRecent().args[1]).toBeTrue();
  });
  ...
  it('should create different marker for position with comment or image', () => {
    ...
    track.positions[positionWithImage].hasImage = true;
    track.positions[positionWithImageAndComment].comment = 'comment 2';
    track.positions[positionWithImageAndComment].hasImage = true;
    spyOn(google.maps.Marker.prototype, 'setIcon');
    spyOn(GoogleMapsApi, 'getMarkerIcon');
    api.map = new google.maps.Map(container);
    // when
    ...
    // then
    expect(GoogleMapsApi.getMarkerIcon).toHaveBeenCalledTimes(1);
    expect(GoogleMapsApi.getMarkerIcon.calls.mostRecent().args[2]).toBeTrue();
    // when
    ...
    expect(GoogleMapsApi.getMarkerIcon.calls.mostRecent().args[2]).toBeFalse();
  });
